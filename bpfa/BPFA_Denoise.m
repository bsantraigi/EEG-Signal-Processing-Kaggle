...
function [ave,Iout,D,S,Z,idex,Pi,NoiseVar,alpha,phi,PSNR,PSNRave] = BPFA_Denoise(IMin, PatchSize, K, InitOption,UpdateOption,IsSubMean, IterPerRound, ReduceDictSize, IMin0,sigma,IMname,DispPSNR)
%------------------------------------------------------------------
...
for colj=1:PatchSize
    for rowi=1:PatchSize
        ...
        clear X_k
        X_k = XUpdate(IMin,PatchSize,idex,MuX);
        [P,N] = size(X_k);
        ...
        if DispPSNR==1 || (rowi==8&&colj==8)            
            ...
            save([IMname,'_Denoising_',InitOption,'_',UpdateOption,'_',num2str(IsSubMean),'_',num2str(sigma),'_',num2str(colj),'_',num2str(rowi)], 'D','Pi','IMin','IMin0','phi','alpha','NoiseVar','idex');
        end
    end
end
